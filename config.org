...
I use [[https://github.com/palantir/python-language-server][pylsp]] as my LSP client. I use [[https://github.com/psf/black][black]] to reformat my Python buffer. Before use it, don't forget to
install =python-black= in your system.

Also, you'll need =python-lsp-server[all]=, =pylsp-black=, =pylsp-isort=, =pylsp-mypy=, =mypy=,
=flake8=.
...
** Rust
rustic is an extension of rust-mode which adds a number of useful features (see
the its github readme) to it. It is the core of the setup and you can use just
it without any other Emacs packages (and without rust-analyzer) if you just want
code highlighting, compilation and cargo commands bound to emacs shortcuts, and
